...
from src.industrial_model import get_industrial_model_candidates
from src.model_evaluation import evaluate_model
from src.config import config, get_project_root

def train_industrial():
    print("=== Training Industrial Fire Model ===")
    
    data_path = get_processed_data_path() / 'industrial_fire_training.csv'
    ...
    candidates = get_industrial_model_candidates()
    best_model = None
    best_name = ""
    best_f1 = -1
    ...
    model_path = models_dir / 'industrial_fire_model.pkl'
    joblib.dump(best_model, model_path)
    print(f"Saved model to {model_path}")
    
    meta_path = models_dir / 'industrial_features.json'
    metadata = {
